...
use ::cursive::views::{BoxView, LinearLayout, OnEventView, Panel, StackView};
use ::cursive::Cursive;
use anyhow::Result;
...
impl View {
    ...
    pub fn run(&mut self) -> Result<()> {
        ...
        self.inner.add_global_callback('?', |s| {
            s.add_fullscreen_layer(BoxView::with_full_screen(
        ...
        self.inner.add_fullscreen_layer(
            StackView::new().fullscreen_layer(BoxView::with_full_screen(
                LinearLayout::vertical()
                    ...
                    .child(
                        OnEventView::new(
                            StackView::new()
                                .fullscreen_layer(BoxView::with_full_screen(
                                    Panel::new(process_view).with_id("process_view_panel"),
                                ))
                                .fullscreen_layer(BoxView::with_full_screen(
                                    Panel::new(cgroup_view).with_id("cgroup_view_panel"),
                                )),
                        )
                        .on_pre_event_inner('p', |stack, _| {
                            let position = stack
                                .find_layer_from_id("process_view_panel")
                        ...
                        .on_pre_event_inner('c', |stack, _| {
                            let position = stack
                                .find_layer_from_id("cgroup_view_panel")
                                .expect("Failed to find cgroup view");
                            stack.move_to_front(position);
                            Some(EventResult::Consumed(None))
                        })
                        .with_id("dynamic_view"),
                    ),
            )),
        );

        self.inner
            .focus_id("dynamic_view")
